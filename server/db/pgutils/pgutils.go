...
func TextOpt(v models.OptionalNullable[string]) pgtype.Text {
	val, ok := v.Get()
	if !ok {
		return pgtype.Text{}
	}
...
func Int4Opt(v models.OptionalNullable[int32]) pgtype.Int4 {
	val, ok := v.Get()
	if !ok {
		return pgtype.Int4{}
	}
...
func UUIDOpt(v models.OptionalNullable[uuid.UUID]) pgtype.UUID {
	val, ok := v.Get()
	if !ok {
		return pgtype.UUID{}
	}
