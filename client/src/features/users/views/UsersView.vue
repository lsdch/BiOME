<template>
  <CRUDTable
    class="fill-height"
    :headers="headers"
    density="compact"
    :items
    :error
    :loading
    @reload="refetch()"
    entityName="User"
    :itemRepr="(p: User) => p.full_name"
    :toolbar="{
      title: 'User',
      icon: 'mdi-account'
    }"
    filter-mode="some"
    appendActions
  >
    <!-- <template #menu>
      <PersonFilters v-model="filters" />
    </template> -->
    <!-- <template #form="props">
      <PersonFormDialogMutation
        @success="props.onSuccess"
        :dialog="props.dialog"
        @update:dialog="(v) => !v && props.onClose()"
        :model-value="props.editItem"
      />
    </template> -->

    <!-- User Role column -->
    <template #[`header.role`]="slotProps">
      <IconTableHeader v-bind="slotProps" icon="mdi-account-badge" />
    </template>
    <template #[`header.organisations`]="slotProps">
      <IconTableHeader v-bind="slotProps" icon="mdi-domain" :expanded="smAndUp" />
    </template>

    <template #[`item.role`]="{ value }">
      <UserRole.Icon :role="value" size="x-small" />
    </template>

    <template #[`item.alias`]="{ value }">
      <span class="font-weight-light"> {{ `@${value}` }}</span>
    </template>
    <template #[`item.organisations`]="{ value }">
      <OrgChip v-for="org in value" :org label size="x-small" />
    </template>

    <template #[`expanded-row-inject`]="{ item }">
      <!-- <v-card v-if="item.comment" flat>
        <v-list-item prepend-icon="mdi-comment">
          {{ item.comment }}
        </v-list-item>
      </v-card> -->
      <v-card flat>
        <v-btn
          v-if="item.contact"
          variant="plain"
          prepend-icon="mdi-at"
          :href="`mailto:${item.contact}`"
          :text="item.contact"
          size="small"
          class="mx-1"
        />
      </v-card>
    </template>
  </CRUDTable>
</template>

<script setup lang="ts">
import { $UserRole, User } from '@/api'

import { UserRole } from '@/api'
import {
  // deletePersonMutation,
  // listPersonsOptions,
  listUsersOptions
} from '@/api/gen/@tanstack/vue-query.gen'
// import PersonFormDialogMutation from '@/components/forms/users/UserFormDialogMutation.vue'
import CRUDTable from '@/components/toolkit/tables/CRUDTable.vue'
import IconTableHeader from '@/components/toolkit/tables/IconTableHeader.vue'
import { useQuery } from '@tanstack/vue-query'
import { computed, ref } from 'vue'
import { useDisplay } from 'vuetify'
// import type { AccountStatus, PersonFilters as Filters } from './PersonFilters.vue'
// import PersonFilters from './PersonFilters.vue'

const { xs, smAndUp } = useDisplay()

// const filters = ref<Filters>({})

// function filterStatus(item: Person, status: AccountStatus) {
//   return status === 'Registered user' ? Boolean(item.role) : item.role === status
// }

// const filter = computed(() => {
//   const { status } = filters.value
//   return (item: Person) => Boolean(!status || filterStatus(item, status))
// })

const { data: items, refetch, error, isPending: loading } = useQuery(listUsersOptions())

const headers: CRUDTableHeader<User>[] = [
  {
    title: 'Role',
    key: 'role',
    width: 0,
    align: 'end',
    sort(a: UserRole, b: UserRole) {
      if (a === b) return 0
      return $UserRole.enum.indexOf(a) - $UserRole.enum.indexOf(b)
    }
  },
  { title: 'Name', key: 'full_name' }
]
</script>

<style scoped></style>
