<template>
  <v-container height="100%" v-if="isFetching || error">
    ...
    <v-alert v-else-if="error" color="error"> Failed to load habitats : {{ error }} </v-alert>
  </v-container>
...
import HabitatFormDialog from '@/components/forms/HabitatGroupFormDialogMutation.vue'
