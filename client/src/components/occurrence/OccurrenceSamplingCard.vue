<template>
  ...
  >
    ...
    <v-sheet :height="250">
      <SitesMap :items="[item.event.site]" />
    </v-sheet>
    ...
    <v-list-item
      ...
      :subtitle="[item.event.site.locality, item.event.site.country?.code].join(', ')"
      :to="{ name: 'site-item', params: { code: item.event.site.code } }"
    >
      <template #subtitle>
        <CountryChip
          v-if="item.event.site.country"
          :country="item.event.site.country"
          size="small"
        />
        {{ item.event.site.locality }}
      </template>
    </v-list-item>
