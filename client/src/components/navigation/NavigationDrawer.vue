<template>
  ...
  >
    <v-list density="compact" nav open-strategy="single">
      <template v-for="(group, key) in navRoutes(settings)" :key>
        <v-divider v-if="isDivider(group)"></v-divider>
        <!-- Route -->
        <template v-else-if="!group.hidden">
          <v-list-item
            v-if="!group.routes"
          ...
          >
            ...
            <template v-for="route in group.routes.filter(({ hidden }) => !hidden)">
...
import { settingsKey } from '@/lib/injection'
...
const settings = inject(settingsKey) as InstanceSettings
