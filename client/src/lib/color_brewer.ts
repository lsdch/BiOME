...
import { Hex, HexToRGB, parseHex, RGB, RGBtoHex } from 'vuetify/lib/util/colorUtils.mjs'
...
const brewerPalettesRGB = Object.fromEntries(
  Object.entries(brewerPalettes).map(([k, palette]) => [
    k,
    palette.map((color) => {
      return HexToRGB(parseHex(color))
    })
  ])
) as { [K in ColorBrewerPaletteKey]: RGB[] }

export function palette(k: ColorBrewerPaletteKey): ColorBrewerPalette {
  return brewerPalettes[k] as ColorBrewerPalette
}

export function paletteRGB(k: ColorBrewerPaletteKey): RGB[] {
  return brewerPalettesRGB[k]
}

export function arrayToHex(color: [number, number, number]): Hex {
  const [r, g, b] = color
  return RGBtoHex({ r, g, b })
}

export function hexToArray(
  color: Hex
