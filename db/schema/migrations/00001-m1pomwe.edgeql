CREATE MIGRATION m1pomweain35ndrk3guaczoj52x4kxgkddbr2ti27e2zmlfx52vzta
    ONTO initial
{
  ...
  CREATE TYPE references::Article EXTENDING default::Auditable {
      CREATE MULTI PROPERTY authors: std::str;
      CREATE PROPERTY comments: std::str;
  ...
  ALTER TYPE events::Sampling {
      ...
      CREATE MULTI LINK reports := (.<sampling[IS occurrence::ExternalBioMat]);
      CREATE MULTI LINK samples := (.<sampling[IS occurrence::InternalBioMat]);
  };
  CREATE TYPE samples::ContentType EXTENDING default::Vocabulary, default::Auditable;
  CREATE ALIAS occurrence::AnyBioMaterial := (
      SELECT
          occurrence::BioMaterial {
              *,
              ...
              external := (IF (occurrence::BioMaterial IS occurrence::ExternalBioMat) THEN [IS occurrence::ExternalBioMat] {
                  original_link := [IS occurrence::ExternalBioMat].original_link,
                  in_collection := [IS occurrence::ExternalBioMat].in_collection,
                  item_vouchers := [IS occurrence::ExternalBioMat].item_vouchers,
                  required quantity := .quantity,
                  content_description := [IS occurrence::ExternalBioMat].content_description
              } ELSE {})
          }
  );
  ...
  CREATE TYPE datasets::Dataset EXTENDING default::Auditable {
  ...
  ALTER TYPE location::Site {
      ...
      CREATE LINK imported_in: datasets::Dataset {
          ON SOURCE DELETE ALLOW;
          ON TARGET DELETE ALLOW;
          CREATE REWRITE
              INSERT 
  ...
  ALTER TYPE datasets::Dataset {
      CREATE MULTI LINK sites := (.<datasets[IS location::Site]);
  };
  ALTER TYPE events::Event {
      CREATE LINK dataset: datasets::Dataset;
